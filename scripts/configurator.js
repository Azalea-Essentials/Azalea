...
world.beforeEvents.itemUse.subscribe(e => {
  system.run(() => {
    if (e.itemStack.typeId == 'azalea:warp') {
      let warps = world.scoreboard.getObjectives().filter(_ => _.id.startsWith('WARP_')).map(_ => _.id.substring(5));
    ...
    if (e.itemStack.typeId == 'azalea:config_ui') {
      ...
      if (e.source.hasTag("experiment-1")) {
        ...
        for (const key of Object.keys(configOptions)) mainForm.button(key, configOptions[key].icon, (player, i) => {
          if (configOptions[key].type) if (configOptions[key].type == "func" && configOptions[key].func) {
            configOptions[key].func(player, i);
            return;
          }
          let configDb = new Database("Config");
          let modalForm = new ModalForm().title(key.split('\n')[0]);
          for (const option of configOptions[key].options) if (option.type == 'text-input') modalForm.textField(option.label, option.placeholder, configDb.get(option.key) ? configDb.get(option.key) : null, (player, text, i) => {
            let configDb = new Database("Config");
            configDb.set(option.key, text);
            ...
            return {
              ...
              callback() {
                let configDb = new Database("Config");
                configDb.set(option.key, option.keyOptions[i]);
              }
            };
          })], option.keyOptions.findIndex(v => v == configDb.get(option.key)) > -1 ? option.keyOptions.findIndex(v => v == configDb.get(option.key)) : 0, (player, selection, i) => {});else if (option.type == 'toggle') modalForm.toggle(option.label, configDb.get(option.key) == "true" ? true : false, (player, state) => {
            let configDb = new Database("Config");
            configDb.set(option.key, state ? 'true' : 'false');
          });
