...
export class Database {
  ...
  get(key, defaultResult = "") {
    ...
    try {
      ...
      let returnData = str.startsWith("OBJECT:") ? JSON.parse(str.substring("OBJECT:".length)) : str.startsWith("NUMBER:") ? parseInt(str.substring("NUM:".length)) : str == "BOOL:true" ? true : str == "BOOL:false" ? false : str;
      let cache = this.getCache();
  ...
  getNoCheck(key) {
    ...
    if (!lenParticipant) return defaultResult;
    let len = objective.getScore(lenParticipant);
    let valParticipants = participants.filter(_ => _.displayName.startsWith(`${key}-`) && !_.displayName.endsWith('L')).sort((a, b) => parseInt(a.displayName.substring(`${key}-`.length) - parseInt(b.displayName.substring(`${key}-`.length))));
  ...
  hardDelete(key) {
    ...
    for (const participant of lenParticipant) objective.removeParticipant(participant);
  }
  getTable() {
    let table = {};
  ...
  get vals() {
    let table = [];
    for (const key of this.gkeys) table.push(this.get(key));
    return table;
  }
  filter(fn) {
...
export class DatabaseNew {
  ...
  keys() {
    return world.getDynamicPropertyIds().filter(_ => _.startsWith(this.table)).map(_ => _.substring(this.table.length + 1));
  }
  delete(key) {
    try {
