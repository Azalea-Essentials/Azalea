  ...
  run(msg, prefix) {
    system.run(() => {
      ...
      if (cmd && cmd.isDev && !msg.sender.hasTag('devmode')) {
        this.callExtensionEvent("internal", "process_response", msg.sender, "ERROR You need to do {{ALT}}/tag @s add devmode {{RESET}}to run this command", theme);
        return;
      }
      ...
      if (cmdStatus == 1 && !isAdmin(msg.sender)) {
        if (!silent) this.callExtensionEvent("internal", "process_response", msg.sender, "ERROR You need to admin to run this command. Try doing {{ALT}}/tag @s add admin{{RESET}}.", theme);
        return;
      }
      if (cmdStatus == 2) {
        if (!silent) this.callExtensionEvent("internal", "process_response", msg.sender, "ERROR This command has been disabled by the admins.", theme);
        return;
      }
      if (cmd && cmdStatus != 3 && cmd.admin && !isAdmin(msg.sender)) {
        if (!silent) this.callExtensionEvent("internal", "process_response", msg.sender, "ERROR You need to admin to run this command. Try doing {{ALT}}/tag @s add admin{{RESET}}.", theme, prefix);
        return;
      }
      let player = msg.sender;
      ...
      if (cmd) {
        ...
        if (!responsesFromExtensions.includes(true)) {
          if (!silent) this.callExtensionEvent("internal", "process_response", msg.sender, "ERROR Command not found. Do {{ALT}}!cmds {{RESET}}to get a list of commands.", theme);
        }
      }
    });
  }
