...
export class ModalForm {
  ...
  dropdown(label, options, defaultValueIndex = 0, callback) {
    ...
    options.forEach((object, i) => {
      if (!(object instanceof Object)) throw new Error(`index: ${i}, in params[1] is not an Object!`);
    });
    const optionStrings = options.map(({
      option
    }, i) => {
      if (typeof option !== 'string') throw new Error(`property option: ${option}, at index: ${i}, in params[1] is not a String!`);
      return option;
    });
    const optionCallbacks = options.map(({
      callback
    }) => {
      if (callback && !(callback instanceof Function)) throw new Error(`property callback at index: ${i}, in params[1] is not a Function!`);else if (callback) return callback;
    });
  ...
  async show(player, awaitNotBusy = false, callback) {
    try {
      ...
      if (cancelationReason !== FormCancelationReason.UserClosed && cancelationReason !== FormCancelationReason.UserBusy) response.formValues.forEach((value, i) => {
