...
export default function() {
    ...
    function openUI(ui, player) {
        ...
        for(const button of ui.buttons) {
            let icon = icons.find(_=>_.name == button.icon);
            
            actionForm.button(button.text, icon && icon.path ? icon.path : undefined, (player,i)=>{
    ...
    uiManager.addUI("Azalea1.1/FormsV2/Root/ManageUI/Buttons/Add", (player, id, formIndex, form, error = "")=>{
        ...
        modalForm.show(player, false, (player, response)=>{
            ...
            if(!response.formValues[1]) return uiManager.open("Azalea1.1/FormsV2/Root/ManageUI/Buttons/Add", player, id, formIndex, form, "Command cant be empty")
            let forms = formsV2Database.get("Forms", []);
            if(!forms || !forms.length) forms = [];
            let form = forms.findIndex(_=>_.id == id);
            forms[form].buttons.push({
                icon: response.formValues[2],
    ...
    uiManager.addUI("Azalea1.1/FormsV2/Root/ManageUI/Buttons/EditProperties", (player, id, formIndex, form, buttonIndex, error = "")=>{
        ...
        ui.show(player, false, (player, response)=>{
            ...
            if(!response.formValues[1]) return uiManager.open("Azalea1.1/FormsV2/Root/ManageUI/Buttons/EditProperties", player, id, formIndex, form, buttonIndex, "Command cant be empty");
            form2.buttons[buttonIndex].icon = response.formValues[2];
            form2.buttons[buttonIndex].command = response.formValues[1];
    ...
    uiManager.addUI("Azalea1.1/FormsV2/Root/ManageUI/Buttons", (player, id)=>{
        ...
        for(let i = 0;i < forms[form].buttons.length;i++) {
            let button = forms[form].buttons[i];
            let iconData = icons.find(_=>_.name == button.icon);
    ...
    return new ConfiguratorSub("§r§6FormsV2", "textures/azalea_icons/FormsV2")
        .setCallback(player => {
            uiManager.open("Azalea1.1/FormsV2/Root", player)
        });
}