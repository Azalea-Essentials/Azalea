  ...
  return (function () {
    ...
    Loki.prototype.configureOptions = function (options, initialConfig) {
      var defaultPersistence = {
        'NODEJS': 'fs',
        'MC': 'dynamic-properties',
        'BROWSER': 'localStorage',
        'CORDOVA': 'localStorage',
        'MEMORY': 'memory'
      },
        persistenceMethods = {
          'dynamic-properties': LokiDynamicPropertyAdapter,
          'fs': LokiFsAdapter,
    ...
    function LokiDynamicPropertyAdapter() {}
    LokiDynamicPropertyAdapter.prototype.loadDatabase = function loadDatabase(dbname, callback) {
      let dynamicProperty = null;
      try {
        dynamicProperty = world.getDynamicProperty(`prismarinev2:${dbname}`);
      } catch { dynamicProperty = null }
      if(!dynamicProperty) dynamicProperty = null
      callback(dynamicProperty);
    }
    LokiDynamicPropertyAdapter.prototype.saveDatabase = function saveDatabase(dbname, dbstring, callback) {
      world.setDynamicProperty(`prismarinev2:${dbname}`, dbstring);
      callback();
    }
    LokiDynamicPropertyAdapter.prototype.deleteDatabase = function deleteDatabase(dbname, callback) {
      world.setDynamicProperty(`prismarinev2:${dbname}`, undefined);
      callback();
    }
    /**
